...
export default function HalamanUtamaAdmin({ initialAdminName = 'Admin' }) {
  ...
  // ✅ CLIENT-SIDE GUARD (fallback): gunakan /api/me?scope=admin
  useEffect(() => {
    let active = true;
    (async () => {
      try {
        const r = await fetch('/api/me?scope=admin', { cache: 'no-store' });
        ...
        setIsLoading(true);
        setError(null);
        const res = await fetch('/api/booking?status=pending', { cache: 'no-store' });
        if (!res.ok) {
  ...
  const totalPages = useMemo(() => Math.max(1, Math.ceil((incomingBookings.length || 0) / itemsPerPage)), [incomingBookings.length, itemsPerPage]);
  ...
  const paginated = useMemo(() => incomingBookings.slice(startIndex, endIndex), [incomingBookings, startIndex, endIndex]);
...
// ✅ SERVER-SIDE GUARD (paling kuat): verifikasi JWT admin sebelum render
export async function getServerSideProps(ctx) {
  const token = ctx.req.cookies?.admin_session || null;

  if (!token) {
    return {
      redirect: {
        destination: `/Signin/hal-signAdmin?from=${encodeURIComponent(ctx.resolvedUrl)}`,
        permanent: false,
      },
    };
  }
